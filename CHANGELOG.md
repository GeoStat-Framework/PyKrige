...
* Updated setup script to provide an option that will disable building Cython extensions during installation. Currently, the Cython extensions are not built if whatever compiler was used to build the running version of Python (usually the Microsoft Visual C Compiler) is not the same as that referred to by Cython distutils build_ext. This should get around problems during installation for Windows users who do not have the Microsoft compiler already installed. Additionally, the Cython extensions currently are not set up to work in Python 3 (see discussion in issue #10 on GitHub), so they are not built when installing with Python 3. This will be changed in the future.