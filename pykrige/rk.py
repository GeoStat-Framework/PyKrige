...
def check_sklearn_model(model, task="regression"):
    ...
    if task == "regression":
        ...
        if not (isinstance(model, BaseEstimator) and isinstance(model, ClassifierMixin)):
            raise RuntimeError(
...
class ClassificationKriging:
    ...
    def __init__(
        ...
        self.n_closest_points = n_closest_points
        self._kriging_kwargs = dict(method=method,
                                    variogram_model=variogram_model,
                                    nlags=nlags,
                                    weight=weight,
                                    n_closest_points=n_closest_points,
    ...
    def fit(self, p, x, y):
        ...
        self.onehotencode = OneHotEncoder(
    ...
    def krige_residual(self, x, **kwargs):
        ...
        krig_pred = [self.krige[i].predict(
            x=x, **kwargs) for i in range(len(self.classes_) - 1)]
